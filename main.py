...
import numpy as np
from FeaturesExtraction import resolution
from FeaturesExtraction import Mobile_UI
from FeaturesExtraction import Contact_info
from FeaturesExtraction import LoadTime
import screenshot
import pickle
import psycopg2
...
def evaluation(new_url):
    ...
    pred_svm = decode_pred_svm(pred_svm)
    ...
    pred_cnn = decode_pred_cnn(pred_cnn)
...
def home():
    ...
    if session["IN"]!=1:
        return render_template('Result.html',data_cnn=pred_cnn,data_svm = pred_svm,reco1=load_timereco,reco2=contact_inforeco,reco3=resreco,reco4=mobilereco,info=1)
    else:
        x=session["Email"]
        x = x.split("@")
        select_users = "SELECT * FROM websites WHERE url = '{}' AND dev_id = '{}' ".format(new_url,session["ID"])
        Record= execute_read_query(connection, select_users)
        if len(Record)==1:
           insert_query = "UPDATE websites SET web_recom1 = '{}', web_recom2 = '{}', web_recom3 = '{}' , web_recom4 = '{}' , cnn_grade = '{}' , svm_grade = '{}' WHERE url = '{}' AND dev_id = '{}'".format(load_timereco,contact_inforeco,resreco,mobilereco,pred_cnn,pred_svm,new_url,session["ID"])
           connection.rollback()
...
def pdf(data_cnn, data_svm, reco1, reco2, reco3, reco4):
    ...
    if session["IN"]!=1:
        return render_template('Result.html',info=1,data_cnn=data_cnn,data_svm = data_svm, reco1 = reco1,reco2=reco2,reco3=reco3, reco4=reco4)
    else:
        x=session["Email"]
        x = x.split("@")
...
def AboutusPage():
    if session["IN"]!=1:
        return render_template('AboutusPage.html',info=1)
    else:
        x=session["Email"]
        x = x.split("@")
...
def MyP():
    ...
    if len(Record)==0:
        return render_template('MyP.html',my_list=[1],info=x[0])
    my_list=[]
    for n in range(len(Record)):
        t,i,z,f,t,d,e,s,u=Record[n]
        my_list.append(i)
    return render_template('MyP.html',my_list=my_list,info=x[0])
...
def login():
    ...
    if len(Record)==1:
        ...
        return render_template('login.html',info='The Email or the Password is not exist')
    if users!=email:
...
# ---------------------------Create an account and add it to the databased---------------------------
...
def signup():
    ...
    Record= execute_read_query(connection, select_users)
    if len(Record)==1:
        return render_template('Sign_up.html',info='The Email is alredy exist')
    else:
...
def loginR(data_cnn, data_svm, reco1, reco2, reco3, reco4):
    ...
    if len(Record)==1:
        ...
        return render_template('Result.html',info2='The Email or the Password is not correct',info=1)
    if users!=email:
        return render_template('Result.html',info2='The Email or the Password is not correct',info=1)
    else:
        session["Email"]=email
